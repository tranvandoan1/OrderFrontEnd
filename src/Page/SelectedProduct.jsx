...
const SelectedProduct = () => {
  ...
  const showModal = () => {
    saveOrders.length >= 1
      ...
      : openNotificationWithIcon("warning", "Bạn chưa chọn món !");
  };
  const handleCancel = () => {
    ...
    <div>
      <div className="order">
                ...
                <Col xs={12} sm={4} md={12} lg={8} xl={8}>
                  <span className="quantity buttons_added">
                    <input
                      type="button"
                      value="-"
                      onClick={() => reduceSaveOrder(item._id)}
                    ...
                    <input
                      type="button"
                      value="+"
                      onClick={() => increasingSaveOrder(item._id)}
        ...
        <div className="discount">
          ...
          <div className="display_ap">
            {value > 0 && `Giảm giá đã chọn: ${value}%`}
          </div>

          <div className="payy">
            <div className="sum">
              Tổng Tiền :{" "}
              <span className="sum_price">
                {(sumPrice == 0 ? sum : sumPrice)
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" "}
                đ
              </span>
          ...
          <Modal visible={success} onBack={Oke}>
            <Result
              ...
              extra={[<Link to={`/floor/floor_id=${floor_id}`}>Quay lại</Link>]}
            />
          </Modal>
        </div>
      </div>
