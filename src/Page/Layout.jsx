...
const LayoutWeb = () => {
    ...
    return (
        <div style={{ backgroundColor: "rgb(243, 243, 243)" }} className={styles.main}>
                    ...
                    <Menu  className={styles.menu} theme="dark" mode="inline" >
                        {
                            floors.map((item, index) => {
                                return (
                                    <Menu.Item key={index} icon={<ApartmentOutlined style={{ color: "black", fontSize: "1.1rem" }} />}>
                                        <NavLink style={{ color: "black" }} to={`/floor/floor_id=${item._id}`}>
                                            {item.name}
                                        </NavLink>
                                    </Menu.Item>
                                )
